...
/*
TEST(remove) {
   char chaine [100] = "il y en a un \n ici";
   AffichageChaine(chaine);
   RemoveEnter(chaine);
   AffichageChaine(chaine);

   CHECK (0 == strcmp(chaine, "il y en a un  ici" ));
...
TEST(Sizeof) {
   ...
   int taille2 = sizeof(int)+100*sizeof(char)+40*sizeof(char); // :-)
...
TEST(lectureFichier) {
   ...
   REQUIRE( 2 == taille );
   CHECK  ( 0 == strcmp(tableau[0].jeu, "2048"));
   CHECK  ( 0 == strcmp(tableau[0].alias, "kiux")); // :-)
   CHECK  ( 64236 == tableau[0].score );
   CHECK  ( 0 == strcmp(tableau[1].jeu, "Minecraft"));
